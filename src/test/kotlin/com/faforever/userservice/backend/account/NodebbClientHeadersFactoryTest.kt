...
class NodebbClientHeadersFactoryTest {

    private val fafProperties: FafProperties = mock()
    private val nodebb: FafProperties.Nodebb = mock()
    private val jwt: FafProperties.Jwt = mock()
    private val hmac: FafProperties.Hmac = mock()
    private val factory = NodebbClientHeadersFactory(fafProperties, HmacService())

    @Test
    fun updateAddsAuthorizationAndHmacHeadersWhenHmacConfigured() {
        whenever(fafProperties.nodebb()).thenReturn(nodebb)
        ...
        assertThat(headers.getFirst(HttpHeaders.AUTHORIZATION), equalTo("Bearer nodebb-token"))
